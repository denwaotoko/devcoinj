...
public abstract class Message implements Serializable {
    ...
    private void selfCheck(byte[] msg, int offset) {
        if (!(this instanceof VersionMessage)) {
            ...
            byte[] reserialized = devcoinSerialize();
    ...
    /**
     * Serialize this message to a byte array that conforms to the devcoin wire protocol.
     * <br/>
    ...
    public byte[] unsafeBitcoinSerialize() {
        ...
        try {
            devcoinSerializeToStream(stream);
        } catch (IOException e) {
        ...
        if (parseRetain) {
            ...
            // which means another call to devcoinSerialize is coming.  If we didn't recache then internal
    ...
    /**
     * Serialize this message to the provided OutputStream using the devcoin wire format.
     *
     * @param stream
     * @throws IOException
     */
    final public void devcoinSerialize(OutputStream stream) throws IOException {
    ...
    /**
     * Serializes this message to the provided stream. If you just want the raw bytes use devcoinSerialize().
     */
    void devcoinSerializeToStream(OutputStream stream) throws IOException {
        log.debug("Warning: {} class has not implemented devcoinSerializeToStream method.  Generating message with no payload", getClass());
    }
