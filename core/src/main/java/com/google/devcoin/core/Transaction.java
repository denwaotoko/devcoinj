 ...
 * <a href="http://code.google.com/p/devcoinj/wiki/WorkingWithTransactions">"Working with transactions"</a> in the
...
public class Transaction extends ChildMessage implements Serializable {
    ...
    // These are serialized in both devcoin and java serialization.
    ...
    public String toString(AbstractBlockChain chain) {
        ...
        for (TransactionOutput out : outputs) {
            ...
            try {
                ...
                s.append(" ");
                s.append(devcoinValueToFriendlyString(out.getValue()));
                s.append(" BTC");
    ...
    public void clearInputs() {
        ...
        this.length = this.devcoinSerialize().length;
    }
    ...
    public synchronized Sha256Hash hashForSignature(int inputIndex, byte[] connectedScript, byte sigHashType) {
        ...
        //
        //   https://en.devcoin.it/wiki/Contracts
    ...
    @Override
    protected void devcoinSerializeToStream(OutputStream stream) throws IOException {
        uint32ToByteStreamLE(version, stream);
        stream.write(new VarInt(inputs.size()).encode());
        for (TransactionInput in : inputs)
            in.devcoinSerialize(stream);
        stream.write(new VarInt(outputs.size()).encode());
        for (TransactionOutput out : outputs)
            out.devcoinSerialize(stream);
     ...
     * This is useful in certain types of <a href="http://en.devcoin.it/wiki/Contracts">contracts</a>, such as
