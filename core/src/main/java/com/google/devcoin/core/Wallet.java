...
import static com.google.devcoin.core.Utils.devcoinValueToFriendlyString;
 ...
 *
 * <p>To learn more about this class, read <b><a href="http://code.google.com/p/devcoinj/wiki/WorkingWithTheWallet">
 ...
 * other objects, see the <a href="http://code.google.com/p/devcoinj/wiki/GettingStarted">Getting started</a> tutorial
 * on the website to learn more about how to set everything up.</p>
 *
 * <p>Wallets can be serialized using either Java serialization - this is not compatible across versions of devcoinj,
...
public class Wallet implements Serializable, BlockChainListener, PeerFilterProvider {
        ...
        try {
            ...
            if (log.isInfoEnabled()) {
                log.info(String.format("Received a pending transaction %s that spends %s BTC from our own wallet," +
                        " and sends us %s BTC", tx.getHashAsString(), Utils.devcoinValueToFriendlyString(valueSentFromMe),
                        Utils.devcoinValueToFriendlyString(valueSentToMe)));
            }
    ...
    private void receive(Transaction tx, StoredBlock block, BlockChain.NewBlockType blockType,
        ...
        log.info("Received tx{} for {} BTC: {} [{}] in block {}", new Object[]{sideChain ? " on a side chain" : "",
                devcoinValueToFriendlyString(valueDifference), tx.getHashAsString(), relativityOffset,
        ...
        if (!insideReorg && bestChain) {
            ...
            log.info("Balance is now: " + devcoinValueToFriendlyString(newBalance));
            if (!wasPending) {
    ...
    public boolean completeTx(SendRequest req) {
        ...
        try {
            ...
            log.info("Completing send tx with {} outputs totalling {} (not including fees)",
                    req.tx.getOutputs().size(), devcoinValueToFriendlyString(value));
            ...
            if (bestChangeOutput != null) {
                ...
                log.info("  with {} coins change", devcoinValueToFriendlyString(bestChangeOutput.getValue()));
            }
            final BigInteger calculatedFee = totalInput.subtract(totalOutput);
            if (calculatedFee.compareTo(BigInteger.ZERO) > 0) {
                log.info("  with a fee of {}", devcoinValueToFriendlyString(calculatedFee));
            }
            ...
            // Check size.
            int size = req.tx.devcoinSerialize().length;
    ...
    private boolean adjustOutputDownwardsForFee(Transaction tx, CoinSelection coinSelection, BigInteger baseFee, BigInteger feePerKb) {
        ...
        int size = tx.devcoinSerialize().length;
    ...
    public String toString(boolean includePrivateKeys, boolean includeTransactions, boolean includeExtensions, AbstractBlockChain chain) {
        ...
        try {
            ...
            builder.append(String.format("Wallet containing %s BTC in:%n", devcoinValueToFriendlyString(getBalance())));
    ...
    private void toStringHelper(StringBuilder builder, Map<Sha256Hash, Transaction> transactionMap,
        ...
        for (Transaction tx : transactionMap.values()) {
            try {
                ...
                builder.append(Utils.devcoinValueToFriendlyString(tx.getValueSentFromMe(this)));
                builder.append(" and receives ");
                builder.append(Utils.devcoinValueToFriendlyString(tx.getValueSentToMe(this)));
                builder.append(", total value ");
                builder.append(Utils.devcoinValueToFriendlyString(tx.getValue(this)));
                builder.append(".\n");
            } catch (ScriptException e) {
    ...
    public void reorganize(StoredBlock splitPoint, List<StoredBlock> oldBlocks, List<StoredBlock> newBlocks) throws VerificationException {
        ...
        try {
            ...
            log.info("post-reorg balance is {}", Utils.devcoinValueToFriendlyString(balance));
        ...
        try {
            ...
            for (ECKey key : keychain) {
                if (key.isEncrypted()) {
                    ...
                    // (all devcoin controlled by that private key is lost forever).
            ...
            for (int i = 0; i < tx.getOutputs().size(); i++) {
                ...
                try {
                    if (out.isMine(this) && out.getScriptPubKey().isSentToRawPubKey()) {
                        ...
                        filter.insert(outPoint.devcoinSerialize());
                    }
                } catch (ScriptException e) {
                    throw new RuntimeException(e); // If it is ours, we parsed the script correctly, so this shouldn't happen
                }
    ...
    private class FeeCalculation {
        ...
        public FeeCalculation(SendRequest req, BigInteger value, List<TransactionInput> originalInputs,
            ...
            while (true) {
                ...
                if (change.compareTo(BigInteger.ZERO) > 0) {
                    ...
                    if (req.ensureMinRequiredFee && Transaction.MIN_NONDUST_OUTPUT.compareTo(change) >= 0) {
                        ...
                        size += changeOutput.devcoinSerialize().length + VarInt.sizeOf(req.tx.getOutputs().size()) - VarInt.sizeOf(req.tx.getOutputs().size() - 1);
                ...
                size += req.tx.devcoinSerialize().length;
            ...
            if (selection3 == null && selection2 == null && selection1 == null) {
                log.warn("Insufficient value in wallet for send: needed {}", devcoinValueToFriendlyString(valueNeeded));
    ...
    private Transaction rekeyOneBatch(long keyRotationTimestamp, final TransactionBroadcaster broadcaster) {
        ...
        try {
            ...
            checkState(rekeyTx.devcoinSerialize().length < Transaction.MAX_STANDARD_TX_SIZE);
            commitTx(rekeyTx);
        } catch (VerificationException e) {
