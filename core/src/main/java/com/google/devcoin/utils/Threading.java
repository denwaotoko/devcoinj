 ...
 * devcoinj performs cycle detection or not. Cycle detection is useful to detect bugs but comes with a small cost.
...
public class Threading {
     ...
     * devcoinj internally shouldn't normally run here, although currently there are a few exceptions.
     */
    ...
    public static void waitForUserCode() {
        ...
        // means there's a bug in devcoinj.
        if (vUserThread != null) {
     ...
     * devcoinj library code is run, setting it after you started network traffic and other forms of processing
    ...
    static {
        ...
        // instantiating any devcoinj objects. The policy change will take effect only on new objects
        ...
        USER_THREAD = Executors.newSingleThreadExecutor(new ThreadFactory() {
            @Nonnull @Override public Thread newThread(@Nonnull Runnable runnable) {
                ...
                t.setName("devcoinj user thread");
                t.setDaemon(true);
