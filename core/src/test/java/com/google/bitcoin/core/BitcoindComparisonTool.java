...
/**
 * A tool for comparing the blocks which are accepted/rejected by devcoind/devcoinj
 * It is designed to run as a testnet-in-a-box network between a single devcoind node and devcoinj
...
public class BitcoindComparisonTool {
    ...
    private static Sha256Hash devcoindChainHead;
    private static volatile Peer devcoind;
    private static volatile InventoryMessage mostRecentInv = null;

    public static void main(String[] args) throws Exception {
        BriefLogFormatter.init();
        System.out.println("USAGE: devcoinjBlockStoreLocation runLargeReorgs(1/0) [port=18444]");
        ...
        // devcoind MUST be on localhost or we will get banned as a DoSer
        ...
        peers.addEventListener(new AbstractPeerEventListener() {
            ...
            public void onPeerDisconnected(Peer peer, int peerCount) {
                ...
                log.error("devcoind node disconnected!");
                System.exit(1);
            }
            ...
            public Message onPreMessageReceived(Peer peer, Message m) {
                if (m instanceof HeadersMessage) {
                    for (Block block : ((HeadersMessage) m).getBlockHeaders())
                        devcoindChainHead = block.getHash();
                    return null;
                } else if (m instanceof Block) {
                    log.error("devcoind sent us a block it already had, make sure devcoind has no blocks!");
                    System.exit(1);
        ...
        devcoindChainHead = params.getGenesisBlock().getHash();
        
        // Connect to devcoind and make sure it has no blocks
        peers.start();
        ...
        for (Rule rule : blockList.list) {
            if (rule instanceof BlockAndValidity) {
                ...
                devcoind.sendMessage(message);
                // devcoind doesn't request blocks inline so we can't rely on a ping for synchronization
                for (int i = 0; !blocksRequested.contains(nextBlock.getHash()); i++) {
                    if (i % 20 == 19)
                        log.error("devcoind still hasn't requested block " + block.ruleName + " with hash " + nextBlock.getHash());
                    Thread.sleep(50);
                }
                devcoind.sendMessage(nextBlock);
                locator.clear();
                locator.add(devcoindChainHead);
                devcoind.sendMessage(new GetHeadersMessage(params, locator, hashTo));
                devcoind.ping().get();
                if (!chain.getChainHead().getHeader().getHash().equals(devcoindChainHead)) {
                    differingBlocks++;
                    log.error("devcoind and devcoinj acceptance differs on block \"" + block.ruleName + "\"");
                }
                ...
                if (mostRecentInv == null && !((MemoryPoolState) rule).mempool.isEmpty()) {
                    log.error("devcoind had an empty mempool, but we expected some transactions on rule " + rule.ruleName);
                    mempoolRulesFailed++;
                } else if (mostRecentInv != null && ((MemoryPoolState) rule).mempool.isEmpty()) {
                    log.error("devcoind had a non-empty mempool, but we expected an empty one on rule " + rule.ruleName);
                    mempoolRulesFailed++;
                    ...
                    if (matches)
                        continue;
                    log.error("devcoind's mempool didn't match what we were expecting on rule " + rule.ruleName);
                    log.info("  devcoind's mempool was: ");
                    for (InventoryItem item : mostRecentInv.items)
                        log.info("    " + item.hash);
        ...
        log.info("Done testing.\n" +
        		"Blocks which were not handled the same between devcoind/devcoinj: " + differingBlocks + "\n" +
