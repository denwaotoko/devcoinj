...
public class WalletTest extends TestWithWallet {
    ...
    public void blockChainCatchup() throws Exception {
        ...
        Threading.waitForUserCode();
        assertEquals(devcoinValueToFriendlyString(wallet.getBalance()), "0.90");
        assertEquals(null, txn[0]);
        assertEquals(2, confTxns.size());
        assertEquals(txn[1].getHash(), send1.getHash());
        assertEquals(devcoinValueToFriendlyString(bigints[2]), "1.00");
        assertEquals(devcoinValueToFriendlyString(bigints[3]), "0.90");
        ...
        assertEquals(devcoinValueToFriendlyString(wallet.getBalance()), "0.80");
        Threading.waitForUserCode();
    ...
    public void balances() throws Exception {
        ...
        Transaction send1 = wallet.createSend(new ECKey().toAddress(params), toNanoCoins(0, 10));
        // Reserialize.
        Transaction send2 = new Transaction(params, send1.devcoinSerialize());
    ...
    public void isConsistent_duplicates() throws Exception {
        ...
        Transaction txClone = new Transaction(params, tx.devcoinSerialize());
        try {
    ...
    public void doubleSpendUnspendsOtherInputs() throws Exception {
        ...
        send2 = new Transaction(params, send2.devcoinSerialize());
    ...
    public void pending1() throws Exception {
        ...
        final Transaction t1Copy = new Transaction(params, t1.devcoinSerialize());
    ...
    public void autosaveImmediate() throws Exception {
        ...
        File f = File.createTempFile("devcoinj-unit-test", null);
    ...
    public void feeSolverAndCoinSelectionTest() throws Exception {
        ...
        assertTrue(request15.tx.devcoinSerialize().length > 1000);
        ...
        assertTrue(request16.tx.devcoinSerialize().length > 1000);
        ...
        int theoreticalMaxLength17 = request17.tx.devcoinSerialize().length + myKey.getPubKey().length + 75;
        ...
        Transaction spend17 = request17.tx;
        {
            ...
            final int length = spend17.devcoinSerialize().length;
        ...
        int theoreticalMaxLength18 = spend18.devcoinSerialize().length + myKey.getPubKey().length + 75;
        ...
        assertTrue(spend18.devcoinSerialize().length >= 998);
        assertTrue(spend18.devcoinSerialize().length <= 1001);
        ...
        assertTrue(request26.tx.devcoinSerialize().length > 1000);
    ...
    public void basicCategoryStepTest() throws Exception {
        ...
        request4.feePerKb = Transaction.REFERENCE_DEFAULT_MIN_TX_FEE.divide(BigInteger.valueOf(request3.tx.devcoinSerialize().length));
        assertTrue(wallet.completeTx(request4));
